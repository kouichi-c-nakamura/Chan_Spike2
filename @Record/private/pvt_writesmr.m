...
p.addParameter('IgnoreStart',false,@(x) isscalar(x) && x == 0 || x == 1 );
...
ignoreStart = p.Results.IgnoreStart;
...
if abs(fix(i64Div_) - i64Div_) < eps('single') %NOTE somehow, even when dInterval is multiple of tBase, i64Div_ is not strictly an integer
    ...
    order = ceil(log10(dInterval_microsec));
    
    tBase_new = dInterval_microsec/10^order * 10^-6;
                
    warning(['The sampling interval of Record object %f microsec was not an integer', ...
        ' multiple of the clock ticks %f microsec (file time base (the length of each tick) [see CEDS64TimeBase.m]). ',...
        'In order to achieve the desired sampling interval, the clock tick has been modified to %f microsec.'],...
        rec.SInterval*10^6,...
        tBase*10^6,...
        tBase_new*10^6)
    tBase = tBase_new;
    
    i64Div = round(rec.SInterval/tBase);
    
    clear order tBase_new
end
