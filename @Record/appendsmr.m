...
% Record.appendsmr allows you to append the channels in a Record object to an
...
%TODO refactor with RecordA.appendsmr
arguments
   rec
...
fid = CEDS64Open(smrfilename,0); % read/write mode for appending
if fid < 0
...
dRate = rec.SRate; % desired rate
dInterval_microsec = 1/dRate * 10^6;
i64Div_ = rec.SInterval/tBase;
...
if abs(fix(i64Div_) - i64Div_) < eps('single') %NOTE somehow, even when dInterval is multiple of tBase, i64Div_ is not strictly an integer
   ...
   error('The time base of the file %s is %f %ssec, but the desired sampling interval %f %ssec is not an integer multiple of the time base.',...
       smrfilename, tBase*10^6, char_greekmu, dInterval_microsec, char_greekmu )
end
            ...
            if iOK == 0
                ...
                if rec.Chans{i}.NSpikes > 0
                    ...
                    for m = 1:rec.Chans{i}.NSpikes
                        ...
                        wmark(m).SetData(waveint16); %TODO where can I specify scale and offset for output???
