...
if nargin <= 2
    ...
    if PNVStart>0
        for i=PNVStart:2:ni
            ...
            Property = varargin{i};
            if i+1>ni
                error('K:RecordAInfo:init:pvsetNoValue', 'no value input for parameter')
            else
                Value = varargin{i+1};
            end
            % Perform assignment
            switch lower(Property)
                ...
                otherwise
                    error('K:RecordAInfo:init:pvsetInvalid', 'invalid parameter')
            end % switch
        end % for
    end
end
...
elseif ischar(chaninfos) && isrow(chaninfos) && ismatchedany(chaninfos, '.mat$')
    % obj = RecordAInfo(matfilename)
    
    chs = Record(chaninfos);
    obj = chs.getRecordAInfo;
    return
else
    error('K:RecordAInfo:init:inputArg:notChanOrChanInfoObj',...
...
%% store ChanInfo objects into RecordAInfo
