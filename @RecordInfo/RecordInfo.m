classdef RecordInfo
    ...
    methods
        %% Constructor
        function obj = RecordInfo(varargin)
            % obj = RecordInfo({ChanInfo1, ChanInfo2, ...})
            % obj = RecordInfo({Chan1, Chan2, ...})
            % obj = RecordInfo()
            % obj = RecordInfo(______, 'Name', 'string')
            % obj = RecordInfo('Name', 'string')
            % obj = RecordInfo(matfilename) %TODO
            %
            % obj = RecordInfo({ChanInfo1, ChanInfo2, ...})
            ...
            %
            % obj = RecordInfo('Name', 'string') creates an empty
            % Record object with a Name.
            %
            % recinfo = RecordInfo(matfilename)  %TODO            
            ...
            if nargin ==1 && isa(varargin{1}, 'RecordInfo') % by itself
                obj = varargin{1};
        ...
        function obj = set.ChanInfos(obj, chaninfos)
            assert( isempty(chaninfos) ||...
                ...
                'K:RecordInfo:set.ChanInfos:chaninfos',...
    ...
    methods (Hidden = true, Access = protected)
        ...
        function out = getSinglePropVal(obj, PropName, defaultval)
            assert(~isempty(PropName) && ischar(PropName) && isrow(PropName) ...
                ...
                'K:RecordInfo:propGetSingleVal:PropName:invalid',...
                'PropName must be valid property name for RecordInfo');
            
            
            errid = sprintf('K:Chan:RecordInfo:%s:invalid', PropName);
